> **Historical snapshot — not current.** Rescued from the abandoned
> `83y5d9-codex/create-project-documentation-readme` branch (commit `49e425a`) so it would not be
> lost when that branch was deleted. It predates both the monorepo redesign and the de-Supabase
> work, so parts are inaccurate: it describes Supabase RPC/Storage coupling and a
> `users_meta` → `auth.users` relationship that no longer exist, references numbered seed files
> (`015_test_seed_core.sql`) that were replaced by `tests/fixtures/`, and lists pgTAP as a future
> enhancement when the suite now has 508 pgTAP tests.
>
> For current reference documentation see `apps/docs/` (the Starlight site).
