...
`smtaCancelInvitation` and `smtaResendInvitation` authorize on organization membership only at the SQL layer — neither requires the caller to be an admin or the original inviter; applications that want that restriction must enforce it themselves via CASL.
