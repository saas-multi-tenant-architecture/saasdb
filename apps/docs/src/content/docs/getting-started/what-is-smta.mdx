...
**SMTA** (SaaS Multi-Tenant Architecture) is a PostgreSQL database design that handles the tenant infrastructure common to every SaaS application: organizations, sub-units, memberships, roles, auditing, and billing. It sits between your platform (Supabase or Payload CMS) and your application tables.
...
<Aside type="note">
Use of a fine-grained, role-based access control (RBAC) system is optional. SMTA can be used as a simple tenant management system without it, and more broad roles like 'admin' and 'user'.
</Aside>
