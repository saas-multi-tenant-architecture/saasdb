...
Soft-deletes a file record. Sets `is_deleted = true` on the record. The file URL and external storage object are not affected. Available to any member of the organization that owns the file.
...
| Condition | Message |
|---|---|
| File does not exist, is already deleted, or caller is not a member of the owning org | `File not found` |
