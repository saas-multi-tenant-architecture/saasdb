...
Invites a user by email. The organization is inferred from the caller's current membership context. Prefer `create_invitation` (in Invitations RPC) for explicit org/unit targeting.
