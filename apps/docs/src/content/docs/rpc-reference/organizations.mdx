...
Soft-deletes a user's membership in an organization. **No super_admin required** — available to any member of that organization.
...
| Condition | Message |
|---|---|
| Caller is not a member of the organization | `Organization not found` |
| The user has no live membership in that organization | `Member not found` |
| The target is the organization's super_admin | `Cannot soft-delete super_admin membership. Transfer super_admin status first.` |
