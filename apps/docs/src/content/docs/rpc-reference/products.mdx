...
Products are created and managed by platform admins directly in the `platform.subscription_products` table — not through a public RPC function. See the **Billing** section for details on how to set up products and connect them to a billing provider (e.g. Stripe).
